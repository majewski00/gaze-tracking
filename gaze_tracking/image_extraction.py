...
class Image:
    ...
    def _extract_eyes(self, img: np.ndarray):
    ...
    def _extract_label(self, img: np.ndarray):

        img = self._random_cropping(img)
        ...
        if results.multi_face_landmarks:
            for _ in results.multi_face_landmarks:
                landmarks_draw = np.array(
                    [np.multiply([p.x, p.y], [225, 225]).astype(int) for p in
                     results.multi_face_landmarks[0].landmark])
        else:
            return None, None

        (l_cx, l_cy), _ = cv2.minEnclosingCircle(landmarks_draw[self.left_iris_cords])
        (r_cx, r_cy), _ = cv2.minEnclosingCircle(landmarks_draw[self.right_iris_cords])

        label = np.array([l_cx / 225., l_cy / 225., r_cx / 225., r_cy / 225.], dtype=np.float16)

        if self.YCrCb:
            img = cv2.cvtColor(img, cv2.COLOR_RGB2YCrCb)

        img = np.array(img, dtype=np.uint8).reshape((224, 224, 3))

        self.face.append(img)
        self.labels.append(label)

    def _transfer_generator(self):
        for face, label in zip(self.face, self.labels):
            if self.mirror:
                imgs = [[face, np.flip(face, axis=1)], [label, abs(np.subtract(label, [1, 0, 1, 0]))]]
            else:
                imgs = [[face], [label]]

            for img, l in zip(imgs[0], imgs[1]):
                yield img / 255., l
    ...
    def create_dataset(self,
                       ...
                       YCrCb: Optional[bool] = False,
                       transfer_learn: Optional[bool] = False):
        """
        Function that load custom Numpy dataset from "path" directory. It automatically perform data augmentation by performing random cropping.

        Args:
            dataset_directory: Name or path to the dataset directory. Directory will contain every .npy file, following project convention.
            shuffle: Boolean value to set shuffling of the dataset
            mirror: Additional data augmentation setting. It will create a mirrored copy of every data point.
            YCrCb: Boolean value indicating whether to use YCrCb color pallet.
            transfer_learn: Special from of dataset generator, containing face image and specific coordinate for simple Transfer Learning.
        Returns:
            TensorFlow Dataset Generator and the size of the dataset
        """
        ...
        if not transfer_learn:
            try:
                self._face_init = np.load(f'{dataset_directory}/face.npy', mmap_mode='r')
                self._binary_face_init = np.load(f'{dataset_directory}/binary_face.npy', mmap_mode='r')
                self._labels_init = np.load(f'{dataset_directory}/labels.npy', mmap_mode='r')
            ...
            self.indices = indices[self.correct_idx]
            ...
            time.sleep(1)
            for idx, face_image in enumerate(self._face_init):
                self._extract_label(img=face_image)
            ...
            dataset = tf.data.Dataset.from_generator(
                ...
                output_signature=((
                                      tf.TensorSpec(shape=(224, 224, 3), dtype=tf.float32, name='face_image')),
                                  tf.TensorSpec(shape=(4,), dtype=tf.float32, name='output')
                )
            )
